...
namespace Plugins.com_DTS.ServingScheduler
{
    ...
    [DisplayName("Blackout Dates Serving ToolBox")]
    [Category("DTS > Serving Scheduler")]
    [Description("Allow Volunteer to set BlackoutDates from ToolBox")]
    ...
    public partial class BlackoutDatesServingToolBox : RockBlock
    {
...
";
        ...
        private string GetPagePersonAliasId()
        {
            ...
            try
            {
                ...
                string query = string.Format("select AliasPersonId from PersonAlias where PersonId = {0} ",this.CurrentPerson.Id);
        ...
        {                        
            try
            {
                ...
                string query = string.Format("select AliasPersoGuid from PersonAlias where PersonId = {0} ",this.CurrentPerson.Id);
                /*
                var mergeFields = GetDynamicDataMergeFields();
        ...
        private DataSet GetData(out string errorMessage, bool schemaOnly = false)
        {
            ...
            query += "AND (ssp.EndDateTime >=GetDate()-1) ";
            query += "Order By ssp.StartDateTime";
