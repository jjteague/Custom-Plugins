...
namespace Plugins.com_DTS.Misc
{
    ...
    [DisplayName( "List Builder Foriegn" )]
    [Category("DTS > ListBuilderForiegn")]
	...
	[ContextAware]
	public partial class ListBuilderByForiegnId : RockBlock
	{
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
			{
				if (comment.InnerText != string.Empty && comment.InnerText.Length > 0)
				{
                    ...
                    //var p2 = rockContext.PersonAliases.Select(x => x.Person).Where(t => 1 == 1).ToList();
                    var persons = rockContext.PersonAliases.Select(x => x.Person).Where(t => t.ForeignId != null && ids.Contains(t.ForeignId.Value)).ToList();
			...
			protected void BindGrid( bool isExporting = false )
				{
					...
					ids = ids.Distinct().ToList();
					var rockContext = new RockContext();
					var persons = rockContext.PersonAliases.Select(x => x.Person).Where(t => ids.Contains(t.Id)).ToList();            
                    gdPerson.DataSource = persons;
					gdPerson.DataBind();
				}
