...
public partial class RoomManagementReport2 : RockBlock
{
    ...
    private void BindGrid()
    {
        ...
        var allLocations = locationsService.ToList().Where( l => l.IsNamedLocation == true );
            ...
            if (numOfRes > 0)
            {
                ...
                foreach (ReservationService.ReservationSummary reservation in reservationSummaryList)
                {
                    ...
                    if (reservation.ReservationStartDateTime.Date == day.Date)
                    {
                        ...
                        reservationStartEndLabel.Text = String.Format( "{0} - {1}", reservation.ReservationStartDateTime.ToString("hh:mm tt"), reservation.ReservationEndDateTime.ToString( "hh:mm tt" ) );
