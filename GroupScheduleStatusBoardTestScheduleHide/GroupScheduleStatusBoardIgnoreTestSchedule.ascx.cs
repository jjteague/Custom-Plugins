...
namespace RockWeb.Plugins.com_DTS.GroupScheduleStatusBoardTestScheduleHide
{
    /// <summary>
    /// 
    /// </summary>
    /// <seealso cref="Rock.Web.UI.RockBlock" />
    [DisplayName( "Group Schedule Status Board Ignoring Test Schedules" )]
    [Category( "com_DTS > Group Scheduling" )]
    [Description( "Scheduler can see overview of current schedules by groups and dates that hides test schedules based on entity attribute" )]
    ...
    [AttributeField("0b2c38a7-d79c-4f85-9757-f1b045d32c8a",
        "Live/Test Schedule Entity Attribute",
        "Attribute on Schedule that indicates whether the schedule is a test schedule or not and therefore should be hidden from the status board",
        false, false, "", "", 0, "TestOrLiveEntityAttribute" )]

    public partial class GroupScheduleStatusBoardIgnoreTestSchedule : RockBlock
    {
        #region Fields
        ...
        private void BuildStatusBoard()
        {
            ...
            // get all the live schedules that are in use by at least one of the GroupLocations
            var testOrLiveScheduleAttribute = new AttributeService(rockContext).Get(GetAttributeValue("TestOrLiveEntityAttribute").AsGuid());

            List<Schedule> groupsScheduleList = new List<Schedule>();

            if (testOrLiveScheduleAttribute != null)
            {
                var groupsScheduleListToTest = groupLocationsQuery.SelectMany(a => a.Schedules).Distinct().AsNoTracking().ToList();
                foreach (var schedule in groupsScheduleListToTest)
                {
                    schedule.LoadAttributes();
                    Rock.Web.Cache.AttributeValueCache value;
                    schedule.AttributeValues.TryGetValue(testOrLiveScheduleAttribute.Key, out value);
                    if (value.Value == true.ToString())
                    {
                        groupsScheduleList.Add(schedule);
                    }
                }
            } else
            {
                groupsScheduleList = groupLocationsQuery.SelectMany(a => a.Schedules).Distinct().AsNoTracking().ToList();
            }
